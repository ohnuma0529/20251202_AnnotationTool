...
YOLOv12による物体検出、SAMによるセグメンテーション、Geminiによる文字起こし機能を統合し、動画からのデータセット作成を支援します。
...
- 信頼度 (Confidence) とサイズ (Size) の閾値調整が可能
...
5. **アノテーション**:
    - 自動検出された魚（Detected Fish）から、保存したいものをクリックして選択します。
...
以下のコマンドを実行すると、`data/videos` 内の全動画をチェックし、未処理のものに対して処理を実行します。
